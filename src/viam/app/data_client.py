...
class DataClient:
    ...
    async def binary_data_by_filter(
        ...
        if dest:
            try:
                file = open(dest, "w")
                file.write(f"{str(response.data)}")
                file.flush()
            except Exception as e:
    ...
    async def binary_data_capture_upload(
        ...
        Args:
            ...
            dataset_ids (Optional[List[str]]): Optional list of dataset IDs to associate with the uploaded data. Data associated with dataset IDs can be filtered by dataset on the Viam app.
